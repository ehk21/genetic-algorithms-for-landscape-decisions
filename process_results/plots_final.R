...
ggplot(dataSummary, aes(y=perc, x=Landclass_ExpOp_GFS, fill=Landclass_ExpOp_GFS)) +
  ...
  geom_errorbar(aes(ymin=perc-sd, ymax=perc+sd), width=.22, position=position_dodge(.6), stat="identity") +
  theme_bw() +
...
pd1 <- pd %>%
  group_by(optim, radius, pop) %>%
  dplyr::summarise(areaMean = mean(patchArea), sd=sd(patchArea))
...
ggplot(pd2, aes(y=areaMean, x=optim)) + #, fill=optim)) +
  ...
  geom_errorbar(aes(ymin=areaMean-sd, ymax=areaMean+sd), width=.22, position=position_dodge(.6), stat="identity") +
  # scale_fill_grey() +
  theme_bw() +
  ...
  geom_text(aes(label=labels, y=areaMean+(sd)), vjust=-.5)+
  ggtitle("A")
...
codes <- read.csv("data/fitness/input_parameters/Landclass_ExpOp_GFS.csv", header=T)
...
# plot normalised objective scores----
...
meanScores <- scoresLong %>%
  ...
  dplyr::summarise(scoreAv=mean(score), scoreSD=sd(score)) # !!!
...
meanScores$normSD <- NA
meanScoresOptim$normScore <- NA
meanScoresOptim$normSD <- NA
...
for(i in 1:nrow(meanScores)){
  ...
  meanScores$normSD[i]<- meanScores$scoreSD[i]/(meanScores$scoreAv[i]*log(10))
  ...
  meanScoresOptim$normSD[i]<-  meanScoresOptim$scoreSD[i]/(meanScoresOptim$scoreAv[i]*log(10))
}
...
ggplot(scoresDF, aes(x=optim, y=normScore, color=objective)) +
  ...
  geom_errorbar(
    aes(ymin=normScore-normSD, ymax=normScore+normSD), width=.3,
  ) +
