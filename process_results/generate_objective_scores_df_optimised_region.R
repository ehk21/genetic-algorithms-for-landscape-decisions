...
source("code/process_results/compute_objective_scores_for_final_landscapes_optim_region.R")
...
landscapeMask <- raster("data/landscapeMask.tif")
...
for(j in rows){
  ...
  print(paste0(which(rows==j), "/", length(rows)))
  ...
  for(i in 1:length(finalRasters)){
    landcoverRaster <- finalRasters[[i]][["optimRaster"]]
    values(landcoverRaster)[which(is.na(values(landcoverRaster)))] <- 0

    marginsRaster <- finalRasters[[i]][["marginsRaster"]]
    values(marginsRaster) <- values(marginsRaster)*values(landscapeMask)
    values(marginsRaster)[which(values(marginsRaster)==0)] <- NA
    
    woodlandRaster <- finalRasters[[i]][["woodlandRaster"]]
    values(woodlandRaster) <- values(woodlandRaster)*values(landscapeMask)
    values(woodlandRaster)[which(values(woodlandRaster)==0)] <- NA
...
# apply mask to rasters so only optimised region is included
values(landcoverRaster) <- values(landcoverRaster) * values(landscapeMask)
values(marginsRaster) <- values(marginsRaster) * values(landscapeMask)
values(woodlandRaster) <- values(woodlandRaster) * values(landscapeMask)
...
# save as csv
write.csv(objectiveScores, "results/objective_scores_optimised_region.csv")
